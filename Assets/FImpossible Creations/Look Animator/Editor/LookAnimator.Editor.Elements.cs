...
namespace FIMSpace.FLook
{
    public partial class FLookAnimator_Editor
    {
        ...
        private void El_DrawAnimateWithSource()
        {
            ...
            if (detected && !Get.SyncWithAnimator) GUI.color = new Color(0.6f, 1f, 0.6f, 1f);

            if (Get.SyncWithAnimator)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.PropertyField(sp_animwithsource, new GUIContent("Sync With Animator", sp_animwithsource.tooltip));
                if (detected && !Get.SyncWithAnimator) GUI.color = c;
                EditorGUILayout.PropertyField(sp_starttpose);
                EditorGUILayout.EndHorizontal();
                GUILayout.Space(7);
                //FGUI_Inspector.DrawUILine(0.07f, 0.1f, 1, 4, 0.99f);
                //EditorGUILayout.BeginHorizontal();
                //if (Get.FixingPreset != FLookAnimator.EFAxisFixOrder.Parental) EditorGUILayout.PropertyField(sp_monitor); else 
                EditorGUIUtility.labelWidth = 156;
                EditorGUILayout.PropertyField(sp_DetectZeroKeyframes); EditorGUIUtility.labelWidth = 0;
                //EditorGUILayout.EndHorizontal();
            }
            else
            {
