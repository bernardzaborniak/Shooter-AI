...
namespace BenitosAI
{
    ...
    public class CI_HS_PrioritiseCorrespondingTP : ConsiderationInput
    {
        ...
        public override float GetConsiderationInput(DecisionContext decisionContext, Consideration consideration)
        {
            ...
            if (currentlyUsedTP == null) return 0;

            if(decisionContext.targetTacticalPoint.tPoint.tacticalPointType == TacticalPointType.CoverPeekPoint)
            {
                if (currentlyUsedTP.tacticalPointType == TacticalPointType.CoverPoint)
                {
                    for (int i = 0; i < currentlyUsedTP.coverPeekPoints.Length; i++)
                    {
                        if (currentlyUsedTP.coverPeekPoints[i] == decisionContext.targetTacticalPoint.tPoint) return 1;
                    }
                }
                else if(currentlyUsedTP.tacticalPointType == TacticalPointType.CoverPeekPoint)
                {
                    ...
                    {
                        if (currentlyUsedTP.coverPointAssignedTo.coverPeekPoints[i] == decisionContext.targetTacticalPoint.tPoint) return 1;
                    }
                }
            }
            else if(decisionContext.targetTacticalPoint.tPoint.tacticalPointType == TacticalPointType.CoverPoint)
            {
                if (currentlyUsedTP.tacticalPointType == TacticalPointType.CoverPeekPoint)
                {
                    if (currentlyUsedTP.coverPointAssignedTo == decisionContext.targetTacticalPoint.tPoint) return 1;
